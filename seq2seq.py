...
import copy
import tensorflow as tf #added by mazhiyuan
...
linear = core_rnn_cell_impl._linear  # pylint: disable=protected-ccess
...
def attention_decoder_(decoder_inputs,
                      initial_state,
                      attention_states,
                      cell,
                      output_size=None,
...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    #outputs = [[] for i in range(beam_size)]
    ...
    for j in range(len(decoder_inputs)):
      ...
      for i, inp in enumerate(decoder_inputs):
        if i > 0:
          variable_scope.get_variable_scope().reuse_variables()
        # If loop_function is set, we use it instead of decoder_inputs
        if loop_function is not None and prev is not None and i>j:
          with variable_scope.variable_scope('loop_function',reuse=True):
            inp = loop_function(prev,i)
        # Merge input and previous attentions into one vector of the right size.
        input_size = inp.get_shape().with_rank(2)[1]
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    #     embedding_classes=num_encoder_symbols,
    #     embedding_size=embedding_size)
    # encoder_inputs = tf.nn.embedding_lookup(embeddings,encoder_inputs)
...
def model_with_buckets(encoder_inputs,
  ...
  # q_cell = tf.contrib.rnn.BasicLSTMCell(128)
  # a_cell = tf.contrib.rnn.BasicLSTMCell(128)
  # w_proj = tf.get_variable('w_proj',shape=[128,128],dtype=tf.float32,initializer=tf.random_uniform_initializer())
  losses = []
  outputs = []
  #qa_weights = []
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
        ...
        # print(j)
        # q,_ = tf.contrib.rnn.static_rnn(q_cell,encoder_inputs[:bucket[0]],dtype=tf.float32,scope='q_rnn')
        # print(1111)
        # a,_ = tf.contrib.rnn.static_rnn(a_cell,decoder_inputs[:bucket[1]],dtype=tf.float32,scope='a_rnn')
        # qa_weights =  tf.nn.sigmoid(tf.reduce_sum(tf.matmul(q[-1],w_proj)*a[-1],axis=1))
        outputs.append(bucket_outputs)
        print('bucket outputs:%d'%(len(bucket_outputs)))
        if per_example_loss:
          loss_per_example = sequence_loss_by_example(
                  outputs[-1][0],
                  targets[:bucket[1]],
                  weights[:bucket[1]],
                  softmax_loss_function=softmax_loss_function)
          # losses.append(tf.losses.compute_weighted_loss(loss_per_example,qa_weights))
          losses.append(loss_per_example)
        else:
          print('outputs[-1][0]:%d'%len(outputs[-1][0]))
          losses.append(
              sequence_loss(
                  outputs[-1][0],
                  targets[:bucket[1]],
